...
scpectrum of clinical isolates. This repository contains all the data and code
to reproduce the analysis. For details and full captions of figures, consult the
publication.
...
fit = vector('list', 9)
names(fit) = unique(dat$strain)
for(i in unique(dat$strain)) {
  for(j in unique(bar$cond)) {
    fit[[i]][[j]] = drm(effect ~ dose, ab, 
      data = subset(bar, strain == i & cond == j), 
...
ci95 = lapply(fit, function(x) lapply(x, confint))  %>% 
  unlist(., recursive = F)  %>% 
  lapply(., as.data.table, keep.rownames = 'par') %>% 
  rbindlist(idcol = T) %>% 
  mutate(.id = sub('\\.', '_', .id)) %>% 
  separate(.id, c('strain', 'cond'), sep = '_') %>% 
  mutate(cond = factor(cond, levels = c('intra', 'pH7.4', 'pH5.5'))) %>% 
  mutate(par = gsub(':|Intercept|\\(|\\)', '', par)) %>% 
  setnames(., c('strain', 'cond', 'par', 'lwr', 'upr'))


pkd = lapply(fit, function(x) lapply(x, coef)) %>% 
  unlist(., recursive = F) %>% 
  do.call(rbind, .) %>% 
  as.data.table(keep.rownames = T) %>% 
  mutate(rn = sub('\\.', '_', rn)) %>% 
  separate(rn, c('strain', 'cond'), '_') %>% 
  mutate(cond = factor(cond, levels = c('intra', 'pH7.4', 'pH5.5'))) %>% 
...
names(pkd) = gsub(':|Intercept|\\(|\\)', '', names(pkd))

foo = pkd %>% 
  pivot_longer(m1:e2, names_to = 'par') %>% 
  left_join(., ci95) %>% 
  mutate(strain = fct_relevel(factor(strain), 'ATCC27853', after = Inf))  %>% 
  mutate(par = factor(par, 
    levels = c('b', 'e1', 'e2', 'm1', 'm2'),
    labels = c('E[min]', 'EC[50]^PMB', 'EC[50]^RIF', 'E[max]^RIF', 'E[max]^PMB'))
    ) %>% 
  as.data.table() %>% 
  left_join(
...
foo[grepl('min', par),        `:=`(min = 0, max = 4)]
foo[grepl('EC\\[50\\]', par), `:=`(min = -2.5, max = 6, 
  value = exp(value), lwr = exp(lwr), upr = exp(upr))]
foo[grepl('max', par),        `:=`(min = -5.5, max = 0)]

myFun = function() {
  plot(fit[[1]][[1]])
  abline(v = c(exp(c(0.5, -1))))
}

p1 = ggplot(foo[!grepl('EC\\[50\\]', par)], aes(value, strain, color = genus)) +
    geom_vline(xintercept = 0, col = 'white') +
    geom_pointrange(aes(xmin = lwr, xmax = upr), size = 1) +
    geom_text(aes(label = round(value, 1)), col = 1, size = 2.5) +
