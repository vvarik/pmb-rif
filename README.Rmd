...
and in liquid medium. Fixed ratio design [Tallarida et al 1997](https://doi.org/10.1016/s0024-3205(97)01030-8).
...
dtk[, group := factor(group, labels=c("neg", "low RIF", "low PMB",
    "low RIF + low PMB", "10x higher RIF", "10x higher PMB", "low PMB", "10x higher PMB"))]
dtk[, outl := isOutlier(effect), .(broth, group, time_h)]
dtk[(outl)]
...
dtk[!(outl)] %>% 
  ...
  geom_line(na.rm=T) +
  geom_pointrange(aes(ymin = response - ci, ymax = response + ci), size=1, fatten=5, fill='white') + 
...
```{r}
dtk %>% 
  ggplot(aes(new_time_h, effect, group=interaction(broth, group), shape=broth, color=outl)) + 
  geom_point(size=2) +
  facet_grid(broth ~ group) 

dtk[group == 'neg' & broth == 'MHBII_pH7.4' & new_time_h == 10]
dtk[group == 'neg' & broth == 'MHBII_pH7.4' & new_time_h == 10]
```
