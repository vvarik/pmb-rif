...
```{r topGO_signif_cell_components}
dat = condition.gene.t.test.results

# GO terms need to be in certain format for topGO
pa14_go = fread('input/ucbpp-pa14_go.csv')
...
# Focus on the genes (alternative would be to take the strongest affected
# mutant of each gene). But that would probably introduce systematic error.
setkey(dat, media, gene)
...
DT::datatable(
  data.table(tab), caption='Table 2. Affected cell components.'
  #options = list(dom = 'ft', ordering=F)
)

cell_com = tab[1:5, c('GO.ID', 'Term', 'raw.p.value')]
# par(cex=0.3)
...
```{r topGO_signif_biological_processes}
...
bio_proc = tab[1:5, c('GO.ID', 'Term', 'raw.p.value')]
...
DT::datatable(
  data.table(tab), caption='Table 3. Affected biological processes.'
  #options = list(dom = 'ft', ordering=F)
)
```
