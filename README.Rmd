...
trace(getTopGO, tracer=browser, at=7)

hits = readRDS('output/rdata/hits.Rdata')
hit_list = unique(hits$PAO1.ortholog)
hit_list = hit_list[!is.na(hit_list)]

# bring in the interactome
filename = '../../../../../../analysis/data/raw/pa_ppi_stringDB_287.protein.links.v11.0.txt'
string_11_pa <- fread(filename)

#divide score by 1000
string_11_pa$combined_score <- string_11_pa$combined_score/1000

#strip / trim species ID
string_11_pa$protein1 <- gsub("287.DR97_", "PA", string_11_pa$protein1, fixed=TRUE)
string_11_pa$protein2 <- gsub("287.DR97_", "PA", string_11_pa$protein2, fixed=TRUE)

# create an igraph object
pa_ppi <- graph.data.frame(string_11_pa, directed = FALSE)
# summary(pa_ppi)
V(pa_ppi)$degree <- degree(pa_ppi)
# table(V(pa_ppi)$degree == 0)

# table(E(pa_ppi)$combined_score>1) # sanity check
# is.simple(pa_ppi)  # is it simple network
pa_ppi_simple <- igraph::simplify(pa_ppi, remove.multiple=T, remove.loops=T, edge.attr.comb='mean')

# summary(pa_ppi)
# summary(pa_ppi_simple)

V(pa_ppi_simple)$degree <- degree(pa_ppi_simple)
# table(V(pa_ppi_simple)$degree == 1)
...
# 0.7 high
# 0.9 very high
# filter combined score at 0.400 (medium)
# table(E(pa_ppi)$combined_score >= 0.4)
# table(E(pa_ppi)$combined_score >= 0.7)
...
# with parameter 'order' set to the number of steps out to go from the query node(s)
#adjacent_vertices(pa_ppi, hit_list, mode="all")
#subgraph(pa_ppi, hit_list)
#induced.subgraph(graph, which(V(graph)$name %in% string_ids))
...
# 0.7 high
# 0.9 very high
# table(E(hit_ppi)$combined_score >= 0.15)
# store degree on the network (main hit network)
V(hit_ppi)$degree <- degree(hit_ppi)

hit_ppi_400 = delete.edges(hit_ppi, which(E(hit_ppi)$combined_score <0.4))
hit_ppi_700 = delete.edges(hit_ppi, which(E(hit_ppi)$combined_score <0.7))

# store degree on the hit_ppi filtered networks
V(hit_ppi_400)$degree <- degree(hit_ppi_400)
# table(V(hit_ppi_400)$degree ==0)

V(hit_ppi_700)$degree <- degree(hit_ppi_700)
# table(V(hit_ppi_700)$degree ==0)


# drop disconnected nodes and evaluate the losses
#hit_ppi_400 = delete.vertices(hit_ppi_400,which(degree(hit_ppi_400)<1))
#hit_ppi_450 = delete.vertices(hit_ppi_450,which(degree(hit_ppi_450)<1))
#hit_ppi_700 = delete.vertices(hit_ppi_700,which(degree(hit_ppi_700)<1))

#hit_ppi_700 = delete.vertices(hit_ppi,which(degree(hit_ppi)<1))
# summary(hit_ppi_400)
# summary(hit_ppi_700)
# table(E(hit_ppi)$combined_score < 0.4)
# table(E(hit_ppi_700)$combined_score < 0.4)

# I'm keeping it at 400 for Cytoscape - can always prune down later

# Count nodes with degree 0
#sum(V(hit_ppi)$degree==0)
#sum(V(hit_ppi_700)$degree==0)
...
lookup = unique(hits[, .(PAO1.ortholog, gene)], by='PAO1.ortholog')
V(hit_ppi)$name = left_join(data.table(PAO1.ortholog=V(hit_ppi)$name), lookup)[, 'gene']


# Node degrees
deg <- degree(hit_ppi_400, mode="all")
#plot(hit_ppi_400, vertex.size=deg*3)
# hist(deg, breaks=1:vcount(hit_ppi_400)-1, main="Histogram of node degree")


# export degree histogram at 400
# pdf("hit_ppi_400_histogram_degree.pdf") 
# G.degree.histogram <- hist(deg, breaks=1:vcount(hit_ppi_400)-1, main="Histogram of node degree")
# dev.off() 
# 
# deg.dist <- degree_distribution(hit_ppi_400, cumulative=T, mode="all")
# plot( x=0:max(deg), y=1-deg.dist, pch=19, cex=1.2, col="orange", 
#       xlab="Degree", ylab="Cumulative Frequency")
# 
# # export degree histogram at 400
# pdf("hit_ppi_400_histogram_degree_cumulative.pdf") 
# plot( x=0:max(deg), y=1-deg.dist, pch=19, cex=1.2, col="orange", 
#       xlab="Degree", ylab="Cumulative Frequency")
# dev.off()
# 
# # degree distribution
# dd <- degree_distribution(hit_ppi_400)
# pdf("hit_ppi_400_degree_distribution.pdf") 
...
# 
# par(mfrow=c(1,2))
#  plot(pa_ppi, vertex.size=hs*500, main="Hubs")
#  plot(pa_ppi, vertex.size=as*300, main="Authorities")
# dev.off()
