---
title: "PA14 Drug interaction screen"
subtitle: "Data analysis results"
...
source("functions/load.iris.files.R")
source("functions/ipak.R")
source("functions/my.strsplit.R")
source("functions/my.t.test.R")
source("functions/surfaceCorrection.correct.outer.rows.columns.separately.multiplicative.R")
source("functions/scale.multiplicative.R")
source("functions/plot.spatial.bias.R")
source("functions/my.table.R")
...
```{r initialize folder structure, warning=F}
#initialize folders ####
dir.create("output/rdata", recursive = T, showWarnings = F)
dir.create("output/pdf", recursive = T, showWarnings = F)
dir.create("output/csv/per.condition.epsilon.phenotypes/", recursive = T, showWarnings = F)
```

```{r load iris files, warning=F}
#load all iris files ####
#and set the right plate number, replicate number and colony
all.iris.files = load.iris.files("input/iris files/")

all.iris.files[,media:=my.strsplit(filename, "-", 1)]
all.iris.files[,antibiotic:=my.strsplit(filename, "-", 2)]
all.iris.files[,condition:=paste(media, antibiotic, sep="-")]

all.iris.files[,plate.id:=
                 filename %>% 
...
all.iris.files[,plate.number:=(plate.id-1)%/%5 + 1]
all.iris.files[,biorep:=(plate.id-1)%%5 + 1]

all.iris.files[,colony:=10000*plate.number + 100*row + column]
all.iris.files[,plate.id:=NULL]


saveRDS(all.iris.files, "output/rdata/all.iris.files.loaded.Rdata")
```

# Introduction
_(email from Nassos)_
Vallo did a combination of `Rifampicin + Polymyxin B` in the PA14 library with
Manu last week (no drug, single drugs, combination). The interaction is a
synergy and we are looking for mutants that resolve it.   

There are also 2 extra concentrations of single drugs (higher) to identify
mutants that are more resistant to single drugs on their own.  
 

There were two media (`LB` and `LB pH 5.5`), five antibiotic treatments
(`rif16`, `pmb2`, `rif16+pmb2`, `rif32`, `pmb4`), and two no-antibiotic
controls. Thus, altogether, 12 unique conditions. We had, indeed, our doubts how
quantitatively one can compare the conditions directly.  

Instead, we aimed for best dynamic range within a plate, and this affected two
conditions which were incubated 3h longer at 37C:  

1. LB pH 5.5 rif16+pmb2, and 
2. LB pH 5.5 rif32.
...
Some plates were either wet, or having patches of swarming colonies.  
These were spotted by eye and removed in the first step, see section `Plate
removal: manual QC`
 

Afterwards, I checked for pairwise replicate correlation.  

Plates that don't match any of their replicates become prime candidates for a
closer look, and possibly removal.  
See section `Replicate correlation checks`
   

After commucating the pairwise correlations to Vallo, he checked the plates
again for plates that would need removal, or can be still salvaged, keeping in
mind we need to have at least 2 replicate plates for the downstream analysis.  
See section `Manual file removal`
   

As a final quality control step, I detect and remove misdetected colonies.  
I reasoned that since colony misdetection is a rare event (~0.05% in other PA
screens), it would be very unlikely that they occur in the same plate position
across replicates.  
In other words, misdetected colonies are likely to have zero size in one of the
replicates, while all other replicates of the same plate do have a colony size.  
See section `Misdetected colony removal`
   


## Plate removal: manual QC

In this step, the eye-spotted bad plates are removed.  
These are the following plates:  

``` {r remove bad plates, warning=F}
#remove bad plates and/or colonies ####

all.iris.files = readRDS("output/rdata/all.iris.files.loaded.Rdata")

bad.plates = 
  fread("input/qc.csv", skip = 3) %>% 
  filter(is.na(row)) %>% 
  #filter(notes=="remove the whole plate")
  filter(!str_detect(notes, "normalize"))

all.iris.files = all.iris.files[!filename %in% bad.plates$file]

bad.colonies = 
  fread("input/qc.csv", skip = 3) %>% 
  filter(!is.na(row)) %>% 
  select(filename=file, row, column) %>% 
  mutate(
    filtered.out=T
  )

all.iris.files = merge(all.iris.files, bad.colonies, 
					   by=c("filename", "row", "column"), all.x=T)
all.iris.files[filtered.out==T, opacity:=NA]
all.iris.files[filtered.out==T, size:=NA]

saveRDS(all.iris.files, "output/rdata/all.iris.files.filtered.Rdata")
	
DT::datatable(bad.plates)
```

Also some colonies that Vallo spotted that Iris had mis-identified or mispinned
etc are removed (173 colonies):  


```{r bad colonies table, warning=F}
fread("input/qc.csv", skip = 3) %>% 
  filter(!is.na(row)) %>% 
  DT::datatable()
```
...
all.iris.files = readRDS("output/rdata/all.iris.files.filtered.Rdata")
...
mutant.all.vs.absent =  merge(
  setnames(my.table(all.iris.files[size==0]$colony), 
           c("colony", "times.absent")),
  setnames(my.table(all.iris.files$colony), 
           c("colony", "all.times")),
  by="colony"
)
...
all.iris.files[size==0 & colony %in% colonys.sometimes.missing, opacity:=NA]

#these are 218 misdetections, or 0.05% of all colonies
dim(all.iris.files[size==0 & colony %in% colonys.sometimes.missing])[1] /
  dim(all.iris.files)[1]

all.iris.files[,opacity.raw:=opacity]

saveRDS(all.iris.files, "output/rdata/all.iris.files.filtered2.Rdata")
```
...
#to other plates)
all.iris.files = readRDS("output/rdata/all.iris.files.filtered2.Rdata")


#get all replicate correlations
#per condition, and per plate number
all.correlations = list()
for(this.condition in unique(all.iris.files$condition)){
  for(this.plate.number in unique(all.iris.files$plate.number)){
    #get all pairwise combinations of the conditions
    this.subset = all.iris.files[condition==this.condition][plate.number==this.plate.number]
...
all.iris.files = all.iris.files[!filename %in% files.to.kick.out$filename]
...
all.iris.files = all.iris.files[!filename %in% files.to.remove.qc2$filename]

saveRDS(all.iris.files, "output/rdata/all.iris.files.filtered3.Rdata")
```
...
# all.iris.files[,opacity.RC.corrected :=
...
# cor(all.iris.files$opacity, all.iris.files$opacity.RC.corrected) #0.798
# #what is the spatial bias before and after?
# all.iris.files[,colony.size:=size] #this function requires an extra column
# plot.spatial.bias(all.iris.files, colnames.to.plot = c("opacity","opacity.RC.corrected"), 
#   output.filename.prefix = "spatial bias", max.point.size = 6, average.metric = "median")
# all.iris.files[,colony.size:=NULL] #we now remove the extra column
# 
...
# column opacity.raw
# all.iris.files[,opacity.raw:=opacity]
# all.iris.files[,opacity:=opacity.RC.corrected]
# 
# saveRDS(all.iris.files, "output/rdata/all.iris.files.outerRC.corrected.Rdata")
```
...
all.iris.files = readRDS("output/rdata/all.iris.files.filtered3.Rdata")

median.to.set = round(median(all.iris.files$opacity, na.rm=T), -2)

all.iris.files[,opacity.plate.corrected:=scale.multiplicative(x=opacity, target.values=median.to.set), by=filename]
all.iris.files[,opacity:=opacity.plate.corrected]

# colors = rep("gray77", all.iris.files$filename %>% unique() %>% length)
# names(colors) = all.iris.files$filename %>% unique()

(ggplot(all.iris.files, aes(x=opacity.raw))+
...
(ggplot(all.iris.files, aes(x=opacity))+
    geom_density(aes(color=filename), alpha=0.2, size=0.15)+
    theme_bw()+
    scale_color_grey()+
    theme(legend.position = "none")+
...
saveRDS(all.iris.files, "output/rdata/all.iris.files.plate.corrected.Rdata")
```
...
#control.opacity
all.iris.files = readRDS("output/rdata/all.iris.files.plate.corrected.Rdata")
...
control.medians = all.iris.files[str_detect(condition, "noAb")] %>% 
...
condition.iris.files = all.iris.files[!str_detect(condition, "noAb")]
